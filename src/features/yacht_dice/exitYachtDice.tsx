...
export const exitYachtDice = async (
  loungeId: string,
  userId: string
): Promise<void> => {
  const reference = fRefrence(database);
  const yachtDiceReference = child(
  ...
  const filteredPlayerIds = yachtDice.playerIds.filter(
    (id: string) => id !== userId
  );

  const updates = {
    [`/${YACHT_DICE_REFERENCE}/${loungeId}/${YACHT_DICE_BOARDS}/${userId}`]:
      null,
    [`/${YACHT_DICE_REFERENCE}/${loungeId}/${YACHT_DICE_PLAYER_IDS}`]:
      filteredPlayerIds,
    [`/${YACHT_DICE_REFERENCE}/${loungeId}/${YACHT_DICE_TURN}`]:
      filteredPlayerIds.length === 0 ? null : yachtDice.turn,
  };
  ...
  if (filteredPlayerIds.length === 0) {
    ...
    const nextPlayerId =
      yachtDice.playerIds[(playerIndex + 1) % yachtDice.playerIds.length];
