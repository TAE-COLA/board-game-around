...
import { type YachtDiceBoard } from '../model';
import React from 'react';
import { Divider } from 'shared/ui';
import { YachtDiceHandButton } from './YachtDiceHandButton';
...
export const YachtDiceHandRanking: React.FC<IProps> = ({
  ...
  const handRankings = {
    ...
    double: {
      name: 'Double',
      check: checkDouble(keptDice),
      marked: board.double.marked,
    },
    triple: {
      name: 'Triple',
      check: checkTriple(keptDice),
      marked: board.triple.marked,
    },
    quadra: {
      name: 'Quadra',
      check: checkQuadra(keptDice),
      marked: board.quadra.marked,
    },
    penta: {
      name: 'Penta',
      check: checkPenta(keptDice),
      marked: board.penta.marked,
    },
    hexa: {
      name: 'Hexa',
      check: checkHexa(keptDice),
      marked: board.hexa.marked,
    },
    fourKind: {
      name: 'Four of a Kind',
      check: checkFourOfAKind(keptDice),
      marked: board.fourKind.marked,
    },
    fullHouse: {
      name: 'Full House',
      check: checkFullHouse(keptDice),
      marked: board.fullHouse.marked,
    },
    smStraight: {
      name: 'Small Straight',
      check: checkSmallStraight(keptDice),
      marked: board.smStraight.marked,
    },
    lgStraight: {
      name: 'Large Straight',
      check: checkLargeStraight(keptDice),
      marked: board.lgStraight.marked,
    },
    yacht: {
      name: 'Yacht',
      check: checkYacht(keptDice),
      marked: board.yacht.marked,
    },
  };
