...
export const exitDavinciCode = async (
  loungeId: string,
  userId: string
): Promise<void> => {
  ...
  const filteredPlayerIds = davinciCode.playerIds.filter(
    (id: string) => id !== userId
  );

  const updates = {
    [`/${DAVINCI_CODE_REFERENCE}/${loungeId}/${DAVINCI_CODE_HANDS}/${userId}`]:
      null,
    [`/${DAVINCI_CODE_REFERENCE}/${loungeId}/${DAVINCI_CODE_PLAYER_IDS}`]:
      filteredPlayerIds,
  ...
  if (filteredPlayerIds.length === 0) {
    updates[`/${DAVINCI_CODE_REFERENCE}/${loungeId}/${DAVINCI_CODE_TURN}`] =
      null;
  } else {
    const playerIndex = davinciCode.playerIds.indexOf(davinciCode.turn);
    const nextPlayerId =
      davinciCode.playerIds[(playerIndex + 1) % davinciCode.playerIds.length];
    updates[`/${DAVINCI_CODE_REFERENCE}/${loungeId}/${DAVINCI_CODE_TURN}`] =
      nextPlayerId;
  }
