...
export const exitLounge = async (
  ...
  const filteredPlayerIds = lounge.playerIds.filter(
    (id: string) => id !== userId
  );
  const updates = {
    [`/${LOUNGE_REFERENCE}/${loungeId}/${LOUNGE_OWNER_ID}`]:
      filteredPlayerIds.length === 0 ? null : filteredPlayerIds[0],
    [`/${LOUNGE_REFERENCE}/${loungeId}/${LOUNGE_PLAYER_IDS}`]:
      filteredPlayerIds,
    [`/${LOUNGE_REFERENCE}/${loungeId}/${LOUNGE_DELETED_AT}`]:
      filteredPlayerIds.length === 0 ? serverTimestamp() : null,
