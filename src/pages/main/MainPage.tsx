  ...
  return (
    <Page loading={loading}>
      <Header>
        <GreetingUser
          onClickLogoutButton={() =>
            onEvent({ type: 'ON_CLICK_LOGOUT_BUTTON' })
          }
        />
      </Header>
      {!state.gameList || state.gameList.length === 0 ? (
        ...
        <GameCardGrid
          ...
          onClickGamePlayButton={(game) =>
            onEvent({ type: 'ON_CLICK_GAME_PLAY_BUTTON', game })
          }
          marginTop='32px'
        />
      )}
      {state.selectedGame && (
        <GameEntryModal
          ...
          onClickCreateLoungeButton={() =>
            onEvent({ type: 'ON_CLICK_CREATE_LOUNGE_BUTTON' })
          }
          onClickJoinLoungeButton={(code) =>
            onEvent({ type: 'ON_CLICK_JOIN_LOUNGE_BUTTON', code })
          }
        />
      )}
    </Page>
