  ...
  const onEvent = async (event: YachtDiceEvent) => {
    switch (event.type) {
      ...
      case 'ON_ROLL_FINISH':
        ...
        await updateYachtDiceState(lounge.id, {
          dice: event.values,
          'rolls-decrease': 1,
        }).catch((error) => {
          if (error.code === 'PERMISSION_DENIED') notMyTurnToast();
        });
        break;
      case 'ON_ADD_DICE_TO_KEEP':
        await updateYachtDiceState(lounge.id, {
          'keep-add': event.index,
        }).catch((error) => {
          if (error.code === 'PERMISSION_DENIED') notMyTurnToast();
        });
        break;
      ...
      case 'ON_CLICK_SELECT_HAND_BUTTON':
        await updateYachtDiceState(lounge.id, {
          boards: { key: event.key, value: event.value },
        }).catch((error) => {
          if (error.code === 'PERMISSION_DENIED') notMyTurnToast();
        });
        break;
