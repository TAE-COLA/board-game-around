...
- API 함수가 커지면 규칙 계산은 `model/*.logic.ts`로 빼고, API는 저장소 읽기/쓰기와 트랜잭션 orchestration에 집중합니다.
- barrel import는 테스트에서 피합니다. 예를 들어 `shared` 대신 `shared/shuffle`, `shared/placeholder`처럼 개별 파일을 직접 import합니다.
...
- `test:the-mind`: The Mind 규칙만 빠르게 확인합니다.
...
- 2/3/4명 The Mind 시작 성공
- 1명/5명 The Mind 시작 실패
